...
if (process.env.MONITORING_ENABLED === 'true' && process.env.SENTRY_DSN) {
    Sentry.init({
        ...
        tracesSampler(samplingContext) {
            ...
            if (
                (samplingContext.request as { url?: string })?.url?.includes('/health') ||
                (samplingContext.request as { url?: string })?.url?.includes('/ready')
            ) {
                return 0;
            }

            // Lower sampling for high-frequency endpoints
            if ((samplingContext.request as { url?: string })?.url?.includes('/sse')) {
        ...
        beforeBreadcrumb(breadcrumb) {
            ...
            if (breadcrumb.category === 'http' && breadcrumb.data) {
                ...
                if ((breadcrumb.data as { headers?: Record<string, string> }).headers) {
                    delete (breadcrumb.data as { headers: Record<string, string> }).headers[
                        'x-pan-token'
                    ];
                    delete (breadcrumb.data as { headers: Record<string, string> }).headers[
                        'authorization'
                ...
                if (
                    (breadcrumb.data as { url?: string }).url?.includes('/health') ||
                    (breadcrumb.data as { url?: string }).url?.includes('/ready')
                ) {
                    return null;
                }
            }
    ...
    // Use console for early initialization message
