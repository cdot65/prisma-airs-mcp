...
import type { ScanRequest, AsyncScanObject } from '../airs/types.js';
import type {
  ScanContentArgs,
...
export class ToolHandler {
    ...
    const tools: Tool[] = [
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            profileName: {
              type: 'string',
              description: 'Name of the AI security profile to use (defaults to "Prisma AIRS")',
            },
            profileId: {
              type: 'string',
  ...
  private async scanAsync(args: Record<string, unknown>): Promise<ToolsCallResult> {
    ...
    const config = getConfig();
    const asyncRequests: AsyncScanObject[] = typedArgs.requests.map((req: AsyncScanRequestItem) => {
      const scanRequest: ScanRequest = {
        ai_profile: {},
        contents: [],
      };
